...
def get_movers(url):
    ...
    rows = soup.select("table tr")[:11]  # כותרת + 10 שורות
    ...
    for row in rows[1:]:
        ...
        if len(cols) >= 3:
            ...
            movers.append((display_name, price, change))
    return movers
...
def format_movers(title, movers, upward=True):
    ...
    for idx, (name, price, change) in enumerate(movers, 1):
        response += f"{idx}. {emoji} {name} – {price} ₪ ({change})\n"
    return response
